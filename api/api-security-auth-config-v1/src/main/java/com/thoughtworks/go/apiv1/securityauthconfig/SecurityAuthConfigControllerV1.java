...
public class SecurityAuthConfigControllerV1 extends ApiController implements SparkSpringController, CrudController<SecurityAuthConfig> {
    ...
    public String index(Request request, Response response) throws IOException {
        SecurityAuthConfigs pluginProfiles = securityAuthConfigService.getPluginProfiles();
        return writerForTopLevelObject(request, response, writer -> SecurityAuthConfigsRepresenter.toJSON(writer, pluginProfiles));
    }
    ...
    public String update(Request request, Response response) {
        ...
        if (isRenameAttempt(securityAuthConfigId, newAuthConfig.getId())) {
            throw haltBecauseRenameOfEntityIsNotSupported("Security auth config");
        }

        if (isPutRequestStale(request, existingAuthConfig)) {
            throw haltBecauseEtagDoesNotMatch("Security auth config", existingAuthConfig.getId());
        }
    ...
    private void haltIfEntityWithSameIdExists(SecurityAuthConfig securityAuthConfig) {
        ...
        securityAuthConfig.addError("id", "Renaming of security auth config IDs is not supported by this API.");
        throw haltBecauseEntityAlreadyExists(jsonWriter(securityAuthConfig), "Security auth config", securityAuthConfig.getId());
    }
}
