...
public class ServerBackupRepository extends HibernateDaoSupport {
    ...
    public void markInProgressBackupsAsAborted(String message) {
        transactionTemplate.execute(status -> {
            List<ServerBackup> inProgressBackups = sessionFactory.getCurrentSession()
                    .createCriteria(ServerBackup.class)
                    .add(Restrictions.eq("status", BackupStatus.IN_PROGRESS))
                    .list();
            inProgressBackups.forEach(eachBackup -> {
                eachBackup
                        .setStatus(BackupStatus.ABORTED)
                        .setMessage(message);
                saveOrUpdate(eachBackup);
            });
        });
    }
