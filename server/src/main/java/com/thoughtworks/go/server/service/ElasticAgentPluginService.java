...
//      messageTimeToLive is lesser than the starvation threshold to ensure there are no duplicate create agent message
        ...
        for (JobPlan plan : plansThatRequireElasticAgent) {
            ...
            if (clusterProfile == null) {
                String cancellationMessage = "\nGoCD canceled the job(%s) because it was scheduled without a cluster profile " +
                        "but job requires the cluster profile to create an elastic agent. Please rerun the job to resolve the issue." +
                        "This will happen if you have upgraded the GoCD server from version prior to 19.3.0 to the current version.";
                final String description = format(cancellationMessage, plan.getIdentifier().buildLocator());
                logToJobConsole(plan.getIdentifier(), description);
