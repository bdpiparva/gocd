...
class ElasticAgentPluginServiceTest {
    ...
    class RescheduleTheJobs {
        ...
        void shouldRescheduleTheJobNotHavingClusterProfile() throws IOException, IllegalArtifactLocationException {
            ...
            verify(consoleService).appendToConsoleLog(jobPlan.getIdentifier(), "\nGoCD canceled the job(pipeline-1/1/stage/1/job) " +
                    "because it was scheduled without a cluster profile but job requires the cluster profile to create an elastic agent. " +
                    "Please rerun the job to resolve the issue.This will happen if you have upgraded the GoCD server from version prior to 19.3.0 to the current version.");
        }
    }
